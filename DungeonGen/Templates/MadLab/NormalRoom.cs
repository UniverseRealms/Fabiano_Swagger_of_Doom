...
namespace DungeonGenerator.Templates.MadLab
{
    internal class NormalRoom : FixedRoom
    {
        ...
        {
            ObjectType = MadLabTemplate.DestructibleWall
        };
        ...
        {
            rasterizer.Copy(MadLabTemplate.MapTemplate, current.Bounds, Pos);

            if ((current.Flags & RoomFlags.Evil) == 0)
                MadLabTemplate.CreateEnemies(rasterizer, Bounds, rand);
            ...
            MadLabTemplate.DrawSpiderWeb(rasterizer, Bounds, rand);
        }
    }
}
