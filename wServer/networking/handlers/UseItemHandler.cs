            ...
            client.Manager.Logic.AddPendingAction(t =>
            {
                ...
                switch (packet.SlotObject.SlotId)
                {
                    case 254:
                        ...
                        {
                            logger.Fatal($"CheatEngine detected for player {client.Player.Name}.\nItem should be Health Potion but it is {item.ObjectId}.");
                            foreach (Player player in client.Player.Owner.Players.Values)
                                if (player.Client.Account.Rank >= 2)
                                    player.SendInfo($"CheatEngine detected for player {client.Player.Name}.\nItem should be Health Potion but it is {item.ObjectId}.");
                            client.Disconnect();
                            return;
                        }
                    ...
                    case 255:
                        ...
                        {
                            logger.Fatal($"CheatEngine detected for player {client.Player.Name}.\nItem should be Magic Potion but it is {item.ObjectId}.");
