...
namespace wServer.realm
{
    ...
    {
        private static readonly ILog logger = LogManager.GetLogger(typeof(RealmPortalMonitor));
        ...
        {
            logger.Info("Initalizing Portal Monitor...");
            ...
            logger.Info("Portal Monitor initialized.");
        }
        ...
        public void WorldAdded(World world)
        {
            lock (worldLock)
            {
                ...
                logger.InfoFormat("World {0}({1}) added to monitor.", world.Id, world.Name);
            }
        }
        ...
        public void WorldClosed(World world)
        {
            lock (worldLock)
            {
                ...
                nexus.LeaveWorld(portal);
                portals.Remove(world);
                logger.InfoFormat("World {0}({1}) closed.", world.Id, world.Name);
            }
        }
