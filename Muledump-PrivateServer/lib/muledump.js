...
var ROW = window.rowlength || 6

// are totals not following equip/inv/vaults options?
var TKGP = false;

var URL = 'http://clarkrealms.net/playerMuledump/view?'
...
function arrangevaults(v) {
    ...
    for (var i = 0; i < VAULTORDER.length; i++) {
        ...
        if (typeof c != 'undefined') r.push(c);
        else r.push(0);
    }
    var w = 7;
    for (var i = 6; i >= 0; i--) {
        for (var j = i; j < r.length; j += w)
            if (r[j]) break;
...
function init_totals() {
    ...
    ids.sort(function(a, b) {
        ...
        function slotidx(it) {
            return SLOT_ORDER.indexOf(it[1])
        }

        function tier(it) {
            return (it[2] < 0) ? 42 : it[2]
        }
...
function toggle_filter() {
    var $self = $(this);
    var id = $self.data('id');
    if (id in filter) delete filter[id];
    else filter[id] = 1;
    relayout();
}
...
function update_filter() {
    ...
    if ($.isEmptyObject(filter) || $('.item.selected:visible').length == 0) {
        for (var i in mules)
            if (mules[i].loaded) mules[i].dom.css('display', 'inline-block');
        return;
    }
    // if filtering
